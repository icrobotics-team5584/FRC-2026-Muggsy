...
bool SubVision::IsEstimateUsable(
  const photon::EstimatedRobotPose& est, std::optional<units::meter_t> precomputedDistance) {
  /* If precomputed distance exists, set avg dist as precomputed distance, else call a function */
  std::optional<units::meter_t> avgDist =
    precomputedDistance ? precomputedDistance : GetAvgDistanceFromCamera(est);
  if (!avgDist) {
    return false;
  }
...
std::optional<int> SubVision::GetClosestTag(frc::Pose2d currentPose) {
  ...
  const std::vector<frc::AprilTag>& tagList = _tagMap.GetTags();

  for (const frc::AprilTag& tag : tagList) {
    int id = tag.ID;
    frc::Pose2d aprilTagPose = tag.pose.ToPose2d();
