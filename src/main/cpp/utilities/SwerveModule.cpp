...
SwerveModule::SwerveModule(
  ...
  if (botVars::GetRobot() == botVars::PRACTICE) {
    ...
    _io = std::make_unique<KrakenIO>(
      canTurnMotorID, canDriveMotorID, canTurnEncoderID, cancoderMagOffset);
  }
  ...
  frc::SmartDashboard::PutNumber(
    "swerve/cancoder " + std::to_string(canTurnEncoderID) + " mag offset",
    cancoderMagOffset.value());

  // Config Motors, dont change order of function calls. Or configs will not be applied before
  // sensors are synced
  ConfigTurnMotor();
  ConfigDriveMotor();
  SyncSensors();
...
void SwerveModule::SetDesiredState(
  frc::SwerveModuleState& referenceState, units::newton_t xForceFF, units::newton_t yForceFF) {
...
void SwerveModule::SendSensorsToDash() {
  ...
  frc::SmartDashboard::PutNumber(
    turnEncoderName + "Abs position", _cancoder.GetAbsolutePosition().GetValue().value());
  frc::SmartDashboard::PutNumber(
    turnEncoderName + "Supply Voltage", _cancoder.GetSupplyVoltage().GetValue().value());
  frc::SmartDashboard::PutString(
    turnEncoderName + "Magnet Health", _cancoder.GetMagnetHealth().GetValue().ToString());
  frc::SmartDashboard::PutNumber(
    turnEncoderName + "Velocity", _cancoder.GetVelocity().GetValue().value());
}
...
void SwerveModule::SetDesiredVelocity(
  units::meters_per_second_t velocity, units::newton_t forceFF) {
