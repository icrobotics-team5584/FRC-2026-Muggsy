...
void LogFalcon(const std::string &name, ctre::phoenix6::hardware::TalonFX& talonFX) {
...
void Log(const std::string &keyName, units::turn_t value) {
  Log(keyName + " (tr)", value.value());
}

void Log(const std::string &keyName, ctre::phoenix6::StatusSignal<units::turn_t>& signal) {
  Log(keyName, signal.GetValue());
}

void Log(const std::string &keyName, units::degree_t value) {
  Log(keyName + " (deg)", value.value());
}

void Log(const std::string &keyName, units::turns_per_second_t value) {
  Log(keyName + " (tps)", value.value());
}

void Log(const std::string &keyName, ctre::phoenix6::StatusSignal<units::turns_per_second_t>& signal) {
  Log(keyName, signal.GetValue());
}

void Log(const std::string &keyName, units::turns_per_second_squared_t value) {
  Log(keyName + " (tps_sq)", value.value());
}

void Log(const std::string &keyName, ctre::phoenix6::StatusSignal<units::turns_per_second_squared_t>& signal) {
  Log(keyName, signal.GetValue());
}

void Log(const std::string &keyName, units::meter_t value) {
  Log(keyName + " (m)", value.value());
}

void Log(const std::string &keyName, units::meters_per_second_t value) {
  Log(keyName + " (mps)", value.value());
}

void Log(const std::string &keyName, units::meters_per_second_squared_t value) {
  Log(keyName + " (mps_sq)", value.value());
}

void Log(const std::string &keyName, units::volt_t value) {
  Log(keyName + " (V)", value.value());
}

void Log(const std::string &keyName, ctre::phoenix6::StatusSignal<units::volt_t>& signal) {
  Log(keyName, signal.GetValue());
}

void Log(const std::string &keyName, units::ampere_t value) {
  Log(keyName + " (A)", value.value());
}

void Log(const std::string &keyName, ctre::phoenix6::StatusSignal<units::ampere_t>& signal) {
  Log(keyName, signal.GetValue());
}

void Log(const std::string &keyName, units::second_t value) {
  Log(keyName + " (s)", value.value());
}

void Log(const std::string &keyName, ctre::phoenix6::StatusSignal<units::second_t>& signal) {
  Log(keyName, signal.GetValue());
}

void Log(const std::string &keyName, units::kilogram_t value) {
  Log(keyName + " (kg)", value.value());
}

void Log(const std::string &keyName, units::celsius_t value) {
  Log(keyName + " (C)", value.value());
}

void Log(const std::string &keyName, ctre::phoenix6::StatusSignal<units::celsius_t>& signal) {
  Log(keyName, signal.GetValue());
}

void Log(const std::string &keyName, frc::Rotation2d value) {
  Log(keyName, value.Degrees());
}

void Log(const std::string &keyName, units::radian_t value) {
  Log(keyName + " (rad)", value.value());
}


double Tune(const std::string &keyName, double defaultValue) {
  double value = defaultValue;

  if (frc::SmartDashboard::ContainsKey(keyName)) {
    value = frc::SmartDashboard::GetNumber(keyName, defaultValue);
  } else {
...
bool Tune(const std::string &keyName, bool defaultValue) {
  bool value = defaultValue;

  if (frc::SmartDashboard::ContainsKey(keyName)) {
    value = frc::SmartDashboard::GetBoolean(keyName, defaultValue);
  } else {
...
std::string Tune(const std::string &keyName, const std::string &defaultValue) {
  ...
  if (frc::SmartDashboard::ContainsKey(keyName)) {
    value = frc::SmartDashboard::GetString(keyName, defaultValue);
  } else {
...
units::turn_t Tune(const std::string &keyName, units::turn_t defaultValue) {
  return units::turn_t(Tune(keyName + " (deg)", defaultValue.value()));
}

units::degree_t Tune(const std::string &keyName, units::degree_t defaultValue) {
  return units::degree_t(Tune(keyName + " (deg)", defaultValue.value()));
}

units::turns_per_second_t Tune(const std::string &keyName, units::turns_per_second_t defaultValue) {
  return units::turns_per_second_t(Tune(keyName + " (tps)", defaultValue.value()));
}

units::turns_per_second_squared_t Tune(const std::string &keyName, units::turns_per_second_squared_t defaultValue) {
  return units::turns_per_second_squared_t(Tune(keyName + " (tps_sq)", defaultValue.value()));
}

units::meter_t Tune(const std::string &keyName, units::meter_t defaultValue) {
  return units::meter_t(Tune(keyName + " (m)", defaultValue.value()));
}

units::meters_per_second_t Tune(const std::string &keyName, units::meters_per_second_t defaultValue) {
  return units::meters_per_second_t(Tune(keyName + " (mps)", defaultValue.value()));
}

units::meters_per_second_squared_t Tune(const std::string &keyName, units::meters_per_second_squared_t defaultValue) {
  return units::meters_per_second_squared_t(Tune(keyName + " (mps_sq)", defaultValue.value()));
}

units::volt_t Tune(const std::string &keyName, units::volt_t defaultValue) {
  return units::volt_t(Tune(keyName + " (V)", defaultValue.value()));
}

units::ampere_t Tune(const std::string &keyName, units::ampere_t defaultValue) {
  return units::ampere_t(Tune(keyName + " (A)", defaultValue.value()));
}

units::second_t Tune(const std::string &keyName, units::second_t defaultValue) {
  return units::second_t(Tune(keyName + " (s)", defaultValue.value()));
}

units::kilogram_t Tune(const std::string &keyName, units::kilogram_t defaultValue) {
  return units::kilogram_t(Tune(keyName + " (kg)", defaultValue.value()));
}

units::celsius_t Tune(const std::string &keyName, units::celsius_t defaultValue) {
  return units::celsius_t(Tune(keyName + " (C)", defaultValue.value()));
}

frc::Rotation2d Tune(const std::string &keyName, frc::Rotation2d defaultValue) {
...
void FieldDisplay::DisplayPose(const std::string& label, frc::Pose2d pose) {
  _fieldDisplay.GetObject(label)->SetPose(pose);
}


void FieldDisplay::DisplayTrajectory(const std::string& name, const frc::Trajectory& trajectory) {
