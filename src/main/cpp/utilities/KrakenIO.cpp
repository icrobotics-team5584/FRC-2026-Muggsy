...
KrakenIO::KrakenIO(int turnCanID, int driveCanID, int encoderCanID, units::turn_t cancoderMagOffset)
  : _canTurnMotor(turnCanID), _canDriveMotor(driveCanID) {}
...
void KrakenIO::ConfigTurnMotor() {
  ...
  _configTurnMotor.MotorOutput.Inverted =
...
void KrakenIO::SetDesiredAngle(units::degree_t angle) {
  _canTurnMotor.SetControl(
    ctre::phoenix6::controls::PositionVoltage(angle).WithEnableFOC(FOCstate));
  _desiredAngle = angle;
}
...
void KrakenIO::SetDesiredVelocity(units::meters_per_second_t velocity, units::newton_t forceFF) {
  ...
  _canDriveMotor.SetControl(
    ctre::phoenix6::controls::VelocityVoltage{(turnsPerSec)}.WithEnableFOC(true).WithFeedForward(
      torqueVoltageFF));

  logger::Log("swerve/drive " + std::to_string(_canDriveMotor.GetDeviceID()) + " torqueVoltage",
    torqueVoltageFF);
...
void KrakenIO::ConfigDriveMotor() {
  _configDriveMotor.Feedback.FeedbackSensorSource =
